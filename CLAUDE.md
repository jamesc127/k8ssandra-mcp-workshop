...
22. (OpenShift) `kubectl apply -f manifests/openshift/k8ssandra-cluster.yaml` mid-workshop SCALES THE RING DOWN → the manifest says `size: 3` because Beat 3 of the talk scales to 6 live. Patch the specific field instead of applying the whole CR
23. Reaper UI shows "no cluster" although `/cluster` returns `demo` → the UI's login token expired (image default `sessionTimeout: PT10M`) and the page silently eats the 401 instead of redirecting to login. The OpenShift CR sets `-Ddw.accessControl.sessionTimeout=PT3H` via `spec.reaper.additionalEnvVars` `JAVA_OPTS`. Changing `spec.reaper` restarts only the Reaper pod, NOT the ring (verified 22 Sep). The UI login is Grafana's (`reaper-ui` secret, copied from `kps-grafana`)
