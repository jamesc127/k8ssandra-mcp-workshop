...
# Reaper signs UI logins with JWT_SECRET, and falls back to a default key that
# is printed in its own config file. Reused if present so a re-run does not
# invalidate open sessions.
if kubectl get secret reaper-jwt -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "    Reusing existing reaper-jwt secret."
else
  kubectl create secret generic reaper-jwt -n "$NAMESPACE" \
    --from-literal=secret="$(openssl rand -hex 48)"
  echo "    Generated reaper-jwt secret."
fi
# Reaper's UI login mirrors Grafana's. Read from the live secret rather than
# $GRAFANA_PASSWORD, which is not what Grafana uses if step 3 skipped the install.
GF_USER=$(kubectl get secret kps-grafana -n monitoring -o jsonpath='{.data.admin-user}' | base64 -d)
GF_PASS=$(kubectl get secret kps-grafana -n monitoring -o jsonpath='{.data.admin-password}' | base64 -d)
kubectl create secret generic reaper-ui -n "$NAMESPACE" \
  --from-literal=username="$GF_USER" \
  --from-literal=password="$GF_PASS" \
  --dry-run=client -o yaml | kubectl apply -f -
echo "    Secret reaper-ui written (same login as Grafana)."
